...
//            $sql .= " inner join members as m on cu.user_id=m.id";
            ...
            $sql = "select m.id,m.realname,cu.company_id,cu.user_id,c.company_name,m.mobile,cu.join_time,cu.login_time,cu.is_admin,";
            $sql .= "sum((case when al.type = 0 and DATE_FORMAT(al.created_at, '%Y%m') = DATE_FORMAT(CURDATE(), '%Y%m') then 1 else 0 end)) as cur_month_d,";
            $sql .= "sum((case when al.type = 0  then 1 else 0 end)) as total_d,";
            $sql .= "sum((case when al.type = 1 and DATE_FORMAT(al.created_at, '%Y%m') = DATE_FORMAT(CURDATE(), '%Y%m') then 1 else 0 end)) as cur_month_c,";
            $sql .= "sum((case when al.type = 1  then 1 else 0 end)) as total_c,";
            $sql .= "(sum((case when al.type = 0 and DATE_FORMAT(al.created_at, '%Y%m') = DATE_FORMAT(CURDATE(), '%Y%m') then 1 else 0 end))+ sum((case when al.type = 0  and PERIOD_DIFF(date_format(now(),'%Y%m'),date_format(al.created_at,'%Y%m'))=1 then 1 else 0 end)))/2  as total_j_d,";
            $sql .= "(sum((case when al.type = 1 and DATE_FORMAT(al.created_at, '%Y%m') = DATE_FORMAT(CURDATE(), '%Y%m') then 1 else 0 end))+ sum((case when al.type = 1  and PERIOD_DIFF(date_format(now(),'%Y%m'),date_format(al.created_at,'%Y%m'))=1 then 1 else 0 end)))/2  as total_j_c";
            $sql .= " from company as c inner join company_user as cu on cu.company_id=c.id inner join members as m on cu.user_id=m.id left join admin_log as al on (cu.company_id = al.company_id and cu.user_id = al.user_id)";
            $sql .= " where 1=1";
            ...
            $sql .= " group by m.id,m.realname,cu.company_id,cu.user_id,c.company_name,m.mobile,cu.join_time,cu.login_time,cu.is_admin";
...
//            $sql .= " inner join members as m on m.id=cu.user_id where cu.is_admin=1";
            ...
            $sql = "select c.id,c.company_name,c.created_at,cu.login_time,m.realname,";
            $sql .= "sum((case when al.type = 0 and DATE_FORMAT(al.created_at, '%Y%m') = DATE_FORMAT(CURDATE(), '%Y%m') then 1 else 0 end)) as cur_month_d,";
            $sql .= "sum((case when al.type = 0 and PERIOD_DIFF(date_format(now(),'%Y%m'),date_format(al.created_at,'%Y%m'))=1 then 1 else 0 end)) as last_month_d,";
            $sql .= "sum((case when al.type = 0  then 1 else 0 end)) as total_d,";
            $sql .= "sum((case when al.type = 1 and DATE_FORMAT(al.created_at, '%Y%m') = DATE_FORMAT(CURDATE(), '%Y%m') then 1 else 0 end)) as cur_month_c,";
            $sql .= "sum((case when al.type = 1 and PERIOD_DIFF(date_format(now(),'%Y%m'),date_format(al.created_at,'%Y%m'))=1 then 1 else 0 end)) as last_month_c,";
            $sql .= "sum((case when al.type = 1  then 1 else 0 end)) as total_c,";
            $sql .= "(sum((case when al.type = 0 and DATE_FORMAT(al.created_at, '%Y%m') = DATE_FORMAT(CURDATE(), '%Y%m') then 1 else 0 end))+ sum((case when al.type = 0  and PERIOD_DIFF(date_format(now(),'%Y%m'),date_format(al.created_at,'%Y%m'))=1 then 1 else 0 end)))/2  as total_j_d,";
            $sql .= "(sum((case when al.type = 1 and DATE_FORMAT(al.created_at, '%Y%m') = DATE_FORMAT(CURDATE(), '%Y%m') then 1 else 0 end))+ sum((case when al.type = 1  and PERIOD_DIFF(date_format(now(),'%Y%m'),date_format(al.created_at,'%Y%m'))=1 then 1 else 0 end)))/2  as total_j_c,";
            $sql .= "(select count(id) from company_user where company_id = c.id) as account_number ";
            $sql .= " from company as c inner join company_user as cu on cu.company_id=c.id inner join members as m on cu.user_id=m.id left join admin_log as al on (cu.company_id = al.company_id and cu.user_id = al.user_id)";
            $sql .= " where cu.is_admin=1";

            if($search){
                $sql .= " and (c.company_name like '%". $search ."%' or m.realname like '%". $search ."%')";
            }

            $sql .= " group by c.id,c.company_name,c.created_at,cu.login_time,m.realname";
